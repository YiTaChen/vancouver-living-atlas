...
export function setFlightCruise(s: FlightState, enabled: boolean) {
  ...
  s.hover = false;
  s.join = true;
  s.climbing = s.y < FLIGHT_LOOP.altitude - 15;
  s.climbYaw = s.yaw;
}
export function setFlightHover(s: FlightState, enabled: boolean) {
  ...
  s.hover = enabled;
  s.cruise = false;
  s.hoverHeight = Math.max(s.y, 2);
  if (enabled) s.power = 0.52;
}
export function crashFlight(s: FlightState) {
  ...
  s.hover = false;
  s.power = 0;
  s.vx = s.vy = s.vz = s.speed = 0;
  s.impact = 1;
}
export function validLanding(
  kind: AircraftKind,
  surface: FlightSurface['kind'],
) {
  return kind === 'helicopter'
    ? surface !== 'water'
    : surface === 'water' || surface === 'beach';
}
...
export function stepFlight(
  ...
  raw: FlightInput,
  dt: number,
  surfaceAt: (x: number, z: number) => FlightSurface,
) {
  dt = clamp(dt, 0, 1 / 30);
  s.age += dt;
  s.impact = Math.max(0, s.impact - dt * 1.7);
  if (s.phase === 'crashed') {
    s.crashAge += dt;
    return;
  }
  let p = clamp(raw.pitch, -1, 1),
    r = clamp(raw.roll, -1, 1),
    yaw = clamp(raw.yaw, -1, 1);
  let targetHeight = 0,
    desiredYaw = s.yaw;
  if (s.cruise) {
    const l = FLIGHT_LOOP,
      t = Math.atan2((s.z - l.z) / l.rz, (s.x - l.x) / l.rx);
    const nearestX = l.x + Math.cos(t) * l.rx,
      nearestZ = l.z + Math.sin(t) * l.rz;
    s.join = Math.hypot(s.x - nearestX, s.z - nearestZ) > 160;
    const look = t + 0.2,
      tx = l.x + Math.cos(look) * l.rx,
      tz = l.z + Math.sin(look) * l.rz;
    ...
    r = clamp(turn * 1.6, -0.8, 0.8);
    yaw = clamp(turn, -0.7, 0.7);
  ...
  if (s.kind === 'seaplane') {
    ...
    s.pitch = damp(
      s.pitch,
      p * 0.43 + (s.phase === 'airborne' ? 0.025 : 0),
      2.0,
      dt,
    );
    s.yaw +=
      ((Math.tan(s.roll) * 9.81) / Math.max(s.speed, 18) +
        yaw * (s.phase === 'grounded' ? 0.48 : 0.15)) *
      dt;
    ...
    s.speed = clamp(
      s.speed +
        (s.power * 6.8 - drag * s.speed - 0.5 - Math.max(0, s.pitch) * 1.5) *
          dt,
      0,
      78,
    );
    ...
    if (s.phase === 'grounded' && (s.speed < 27 || s.pitch < 0.055))
      vertical = 0;
    s.vy = damp(s.vy, vertical, 1.8, dt);
    s.vx = Math.sin(s.yaw) * s.speed;
    s.vz = -Math.cos(s.yaw) * s.speed;
  } else {
    s.stalled = false;
    if (s.cruise) {
      const turn = angle(desiredYaw - s.yaw);
      s.yaw += clamp(turn * 0.9, -0.42, 0.42) * dt;
      const speed = s.climbing ? 0 : 40;
      s.vx = damp(s.vx, Math.sin(s.yaw) * speed, 0.6, dt);
      s.vz = damp(s.vz, -Math.cos(s.yaw) * speed, 0.6, dt);
      s.vy = damp(s.vy, clamp((targetHeight - s.y) * 0.28, -5, 7), 1, dt);
      s.pitch = damp(s.pitch, -0.13, 2, dt);
      s.roll = damp(s.roll, r * 0.3, 2, dt);
    } else {
      s.pitch = damp(s.pitch, p * 0.35, 2.6, dt);
      s.roll = damp(s.roll, r * 0.4, 2.6, dt);
      s.yaw += yaw * 0.75 * dt;
      const forward = -Math.sin(s.pitch) * 14,
        side = Math.sin(s.roll) * 12;
      s.vx +=
        (Math.sin(s.yaw) * forward + Math.cos(s.yaw) * side - s.vx * 0.13) * dt;
      s.vz +=
        (-Math.cos(s.yaw) * forward + Math.sin(s.yaw) * side - s.vz * 0.13) *
        dt;
      if (s.hover) {
        s.vx = damp(s.vx, 0, 1.7, dt);
        s.vz = damp(s.vz, 0, 1.7, dt);
        s.vy = damp(s.vy, clamp((s.hoverHeight - s.y) * 1.3, -3, 3), 2, dt);
      } else if (raw.descend) {
        s.vy = damp(s.vy, -2.2, 2, dt);
      } else s.vy += ((s.power - 0.52) * 18 - s.vy * 0.85) * dt;
    }
    s.vy = clamp(s.vy, -9, 10);
    s.speed = Math.hypot(s.vx, s.vz);
    if (s.phase === 'grounded' && s.vy <= 0) {
      s.vx = damp(s.vx, 0, 8, dt);
      s.vz = damp(s.vz, 0, 8, dt);
    }
  }
  s.x += s.vx * dt;
  s.z += s.vz * dt;
  ...
  if (s.y <= next.height) {
    ...
    if (!validLanding(s.kind, next.kind) || downward > 11) {
      crashFlight(s);
      return;
    }
