...
## Lions Gate pedestrian railing correction — 2026-09-06

Replaced generic sparse bridge footway rails with continuous green top/bottom members, dense vertical pickets and stronger posts, using the actual mitered walkway edge instead of fallible endpoint floor lookups. Only outer footway rails are emitted; the road-side barrier now has a narrow tapered concrete profile and concrete material. Both original west tower openings and the road entrance opening remain. No road/walk floor datum or navigation changes. Geometry audit finds zero shared-endpoint error and no unplanned gaps; 348 tests, TypeScript and 58 canonical Causeway checks pass. Actual east/west Walk-height captures and bounded High metrics are in `docs/visual-quality/lions-railings/README.md`.
